...
describe('How the provider should work', function() {
  ...
  it('frequently calling reset, start and complete should work', function() {
    ...
    waits(10);
    runs(function() {
      expect([1, this.progressbar.status()]).toEqual([1, 100]);
    });
    ...
    waits(10);
    runs(function() {
      expect([2, this.progressbar.status()]).toEqual([2, 100]);
    });
    ...
    waits(10);
    runs(function() {
      expect([3, this.progressbar.status()]).toEqual([3, 100]);
    });

    waits(200);
    runs(function() {
      expect([4, this.progressbar.status()]).toEqual([4, 100]);
    });

    waits(200);
    runs(function() {
      expect([5, this.progressbar.status()]).toEqual([5, 100]);
    });

    waits(200);
    runs(function() {
      expect([6, this.progressbar.status()]).toEqual([6, 100]);
    });
  });
  ...
  it('set update count interval', function() {
    var defaultInterval = this.progressbar.updateCountInterval;

    // default interval is minimum possible value
    this.progressbar.setUpdateInterval(defaultInterval - 10);
    expect(this.progressbar.updateCountInterval).toBe(defaultInterval);

    this.progressbar.setUpdateInterval(defaultInterval + 10);
    expect(this.progressbar.updateCountInterval).toBe(defaultInterval + 10);
  });

  it('set update count rate', function() {
    var defaultUpdateCountRate = this.progressbar.updateCountRate;

    // rate can not be zero
    this.progressbar.setUpdateRate(0);
    expect(this.progressbar.updateCountRate).toBe(defaultUpdateCountRate);

    // negative value is converted to positive
    this.progressbar.setUpdateRate(-15);
    expect(this.progressbar.updateCountRate).toBe(15);

    this.progressbar.setUpdateRate(defaultUpdateCountRate + 10);
    expect(this.progressbar.updateCountRate).toBe(defaultUpdateCountRate + 10);
  });
});
