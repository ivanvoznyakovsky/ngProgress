...
angular.module('ngProgress.provider', ['ngProgress.directive'])
  .service('ngProgress', function() {
    ...
    return ['$document', '$window', '$compile', '$rootScope', '$timeout', function($document, $window, $compile, $rootScope, $timeout) {
      ...
      this.count = 0;
      this.updateCountInterval = 200;
      this.updateCountRate = 0.15;
      ...
      this.setUpdateRate = function(rate) {
        var normalizedRate = rate ? Math.abs(parseFloat(rate)) : 0;
        this.updateCountRate = normalizedRate || this.updateCountRate;
      };

      this.setUpdateInterval = function(interval) {
        var normalizedInterval = interval ? Math.abs(parseInt(interval, 10)) : 0;
        this.updateCountInterval = normalizedInterval >= 200 ? normalizedInterval : this.updateCountInterval;
      };
